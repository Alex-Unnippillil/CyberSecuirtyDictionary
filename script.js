  ...
  termsData.terms
    ...
    .forEach((item) => {
      ...
      if (matchesSearch && matchesFavorites && matchesLetter) {
        ...
        star.setAttribute("aria-describedby", "favorite-desc");
        const initialFavorited = favorites.has(item.term);
        star.textContent = "★";
        if (initialFavorited) {
          star.classList.add("favorited");
        }
        star.setAttribute("aria-pressed", initialFavorited);
        star.setAttribute(
          "aria-label",
          initialFavorited ? "Remove from favorites" : "Add to favorites",
        );
        star.addEventListener("click", (e) => {
          e.stopPropagation();
          toggleFavorite(item.term);
          const favorited = favorites.has(item.term);
          star.classList.toggle("favorited");
          star.setAttribute("aria-pressed", favorited);
          star.setAttribute(
            "aria-label",
            favorited ? "Remove from favorites" : "Add to favorites",
          );
          if (liveRegion) {
            liveRegion.textContent = favorited
              ? `${item.term} added to favorites`
              : `${item.term} removed from favorites`;
          }
