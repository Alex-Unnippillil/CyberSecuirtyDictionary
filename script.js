...
const cacheBtn = document.getElementById("cache-collection");
const cacheSelect = document.getElementById("collection-select");
...
const cacheCollections = {
  core: ["/", "/index.html", "/styles.css", "/script.js", "/terms.json"],
  terms: [],
};
...
function updateCacheMeta() {
  if (!cacheSelect || !cacheMeta) return;
  const name = cacheSelect.value;
  const meta = JSON.parse(localStorage.getItem("cache-meta-" + name) || "null");
  if (meta && meta.completed) {
    const kb = (meta.size / 1024).toFixed(1);
    cacheMeta.textContent = `Cached ${kb} KB`;
  } else {
    cacheMeta.textContent = "Not cached";
  }
}

function initCacheCollections() {
  if (!cacheSelect) return;
  cacheCollections.terms = termsData.terms.map(
    (t) => `/terms/${slugify(t.term)}.html`,
  );
  updateCacheMeta();
}

if (cacheBtn && cacheSelect && cacheProgress && cacheMeta) {
  cacheSelect.addEventListener("change", updateCacheMeta);
  cacheBtn.addEventListener("click", () => {
    const name = cacheSelect.value;
    const urls = cacheCollections[name];
    if (!urls || !urls.length) return;
    cacheProgress.value = 0;
    cacheProgress.max = urls.length;
    cacheProgress.hidden = false;
    navigator.serviceWorker.ready.then((reg) => {
      reg.active?.postMessage({ type: "CACHE_COLLECTION", name, urls });
    });
  });

  navigator.serviceWorker.addEventListener("message", (event) => {
    const data = event.data;
    if (!data || data.name !== cacheSelect.value) return;
    if (data.type === "CACHE_PROGRESS") {
      cacheProgress.value = data.index;
    } else if (data.type === "CACHE_COMPLETE") {
      cacheProgress.hidden = true;
      localStorage.setItem(
        "cache-meta-" + data.name,
        JSON.stringify({ size: data.size, completed: true }),
      );
      updateCacheMeta();
    }
  });
