...
if (mainContainer) {
  ...
  function handlePointerMove(e) {
    if (usingKeyboard || motionQuery.matches) {
      return;
    }
    ...
    const maxDeg = 6;
    const rotateY = Math.max(Math.min(xRatio * 12, maxDeg), -maxDeg);
    const rotateX = Math.max(Math.min(-yRatio * 12, maxDeg), -maxDeg);
    mainContainer.style.transform = `perspective(600px) rotateX(${rotateX}deg) rotateY(${rotateY}deg)`;
  }

  mainContainer.addEventListener("pointermove", handlePointerMove);
  mainContainer.addEventListener("pointerleave", resetTilt);
