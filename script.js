...
const lowMotionIndicator = document.getElementById("low-motion-indicator");
...
scrollBtn.addEventListener("click", () =>
  window.scrollTo({
    top: 0,
    behavior: document.body.classList.contains("low-motion")
      ? "auto"
      : "smooth",
  }),
);
...
function applyLowMotion(enabled) {
  if (enabled) {
    document.body.classList.add("low-motion");
    if (lowMotionIndicator) {
      lowMotionIndicator.hidden = false;
    }
  } else {
    document.body.classList.remove("low-motion");
    if (lowMotionIndicator) {
      lowMotionIndicator.hidden = true;
    }
  }
}

function handleLowMotionChange() {
  const prefersReduced = reducedMotionQuery.matches;
  const saveData = navigator.connection && navigator.connection.saveData;
  applyLowMotion(prefersReduced || saveData || batterySaver);
}
...
if ("getBattery" in navigator) {
  navigator.getBattery().then((battery) => {
    function updateBattery() {
      batterySaver = !battery.charging && battery.level < 0.2;
      handleLowMotionChange();
    }
