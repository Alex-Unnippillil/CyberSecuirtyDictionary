...
const COPY_PRESET_KEY = "copyPreset";
...
function getStoredPreset() {
  return localStorage.getItem(COPY_PRESET_KEY) || "slack";
}

function generateClipboardContent(term, definition, preset) {
  switch (preset) {
    case "markdown":
      return `**${term}**: ${definition}`;
    case "html":
      return `<strong>${term}</strong>: ${definition}`;
    case "slack":
    default:
      return `*${term}*: ${definition}`;
  }
...
function displayDefinition(term) {
  ...
  definitionContainer.innerHTML = `
    ...
    <div class="copy-controls">
      <button id="copy-button" type="button" aria-label="Copy definition">Copy</button>
      <label>Preset:
        <select id="copy-preset" aria-label="Copy preset">
  ...
  const presetSelect = document.getElementById("copy-preset");
  const copyBtn = document.getElementById("copy-button");
  const controls = definitionContainer.querySelector(".copy-controls");
  if (presetSelect) {
    presetSelect.value = storedPreset;
    presetSelect.addEventListener("change", (e) => {
      e.stopPropagation();
      localStorage.setItem(COPY_PRESET_KEY, presetSelect.value);
    });
  ...
  if (copyBtn) {
    copyBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      const preset = presetSelect ? presetSelect.value : storedPreset;
      ...
      localStorage.setItem(COPY_PRESET_KEY, preset);
    });
  }
  if (controls) {
