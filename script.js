...
function displayDefinition(term) {
  ...
  currentTermIndex = filteredTerms.findIndex((t) => t.term === term.term);
}

function navigateTerm(offset) {
  if (!filteredTerms.length || currentTermIndex === -1) return;
  const newIndex = currentTermIndex + offset;
  if (newIndex < 0 || newIndex >= filteredTerms.length) return;
  displayDefinition(filteredTerms[newIndex]);
}
...
definitionContainer.addEventListener("touchstart", (e) => {
  if (e.touches.length !== 1) return;
  const touch = e.touches[0];
  if (
    touch.clientX < EDGE_GAP ||
    touch.clientX > window.innerWidth - EDGE_GAP
  ) {
    swipeActive = false;
    return;
  }
...
definitionContainer.addEventListener("touchend", (e) => {
...
definitionContainer.addEventListener("touchcancel", () => {
  swipeActive = false;
});
