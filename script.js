...
const cacheMetadata = document.getElementById("cache-metadata");
...
if (cacheBtn && "serviceWorker" in navigator) {
  ...
  try {
    ...
    if (stored) {
      ...
      if (meta.completed) {
        cacheMetadata.textContent = `Cached ${url} - ${(meta.size / 1024).toFixed(1)} KB`;
      }
    }
  } catch (e) {
    // Ignore parse errors
  ...
  cacheBtn.addEventListener("click", () => {
    ...
    channel.port1.onmessage = (event) => {
      ...
      if (data.type === "CACHE_PROGRESS" && data.total) {
        cacheProgress.value = (data.received / data.total) * 100;
      } else if (data.type === "CACHE_COMPLETE") {
        cacheProgress.value = 100;
        cacheProgress.style.display = "none";
        const meta = { size: data.size, completed: true };
        try {
          localStorage.setItem(metaKey, JSON.stringify(meta));
        } catch (e) {
          // Ignore storage errors
        }
        cacheMetadata.textContent = `Cached ${url} - ${(data.size / 1024).toFixed(1)} KB`;
      } else if (data.type === "CACHE_ERROR") {
        cacheProgress.style.display = "none";
        cacheMetadata.textContent = `Error caching ${url}: ${data.error}`;
      }
    };
    navigator.serviceWorker.controller.postMessage(
      { type: "CACHE_COLLECTION", url },
      [channel.port2],
    );
  });
}
