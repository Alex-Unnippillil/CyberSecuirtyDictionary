...
const navRandomButton = document.getElementById("nav-random");
...
if (navRandomButton) {
  navRandomButton.addEventListener("click", async () => {
    try {
      navRandomButton.disabled = true;
      const response = await fetch("/api/random-word");
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const data = await response.json();
      const slug = data.slug || data.word || data;
      document.body.classList.add("fade-out");
      setTimeout(() => {
        const url = slug.startsWith("/") ? slug : `terms/${slug}.html`;
        window.location.href = url;
      }, 500);
    } catch (error) {
      console.error("Failed to fetch random word:", error);
    } finally {
      navRandomButton.disabled = false;
    }
  });
}
