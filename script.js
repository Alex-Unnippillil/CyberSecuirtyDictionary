...
function applyReduceMotionSetting() {
  ...
  const shouldReduce =
    stored === "true" || (stored === null && reduceMotionQuery.matches);
  document.body.classList.toggle("reduce-motion", shouldReduce);
  if (reduceMotionToggle) {
    reduceMotionToggle.setAttribute("aria-pressed", shouldReduce);
  }
}

applyReduceMotionSetting();

reduceMotionQuery.addEventListener("change", applyReduceMotionSetting);
