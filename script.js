...
if (showFavoritesToggle) {
  showFavoritesToggle.addEventListener("change", () => {
    ...
    populateTermsList();
    if (addToCalendarBtn) {
      addToCalendarBtn.style.display = showFavoritesToggle.checked
        ? "inline-block"
        : "none";
    }
  });
...
if (addToCalendarBtn) {
  addToCalendarBtn.addEventListener("click", () => {
    const terms = Array.from(favorites);
    if (!terms.length) {
      alert("No favorite terms selected.");
      return;
    }
    const icsContent = generateReviewICS(terms);
    const blob = new Blob([icsContent], { type: "text/calendar" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "cybersecurity-review.ics";
