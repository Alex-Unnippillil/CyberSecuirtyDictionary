...
function displayDefinition(term) {
  ...
  const regions = Array.from(
    new Set(examples.map((e) => e.region).filter(Boolean)),
  );

  const regionBadges = regions
    .map(
      (r) => `<span class="badge region-badge" data-region="${r}">${r}</span>`,
    )
    .join("");

  const examplesList = examples
    .map((e) => `<li class="example" data-region="${e.region}">${e.text}</li>`)
    .join("");

  definitionContainer.innerHTML = `
    <div class="headword-bar">
      <h3>${term.term}</h3>
      ${regionBadges ? `<div class="region-badges">${regionBadges}</div>` : ""}
    </div>
    <p>${term.definition}</p>
  ...
  const exampleEls = definitionContainer.querySelectorAll(".example");
  const badgeEls = definitionContainer.querySelectorAll(".region-badge");

  function filterExamples(region) {
    exampleEls.forEach((ex) => {
      ex.style.display = !region || ex.dataset.region === region ? "" : "none";
    });
  }
  ...
  badgeEls.forEach((badge) => {
    badge.addEventListener("click", (e) => {
      ...
      badgeEls.forEach((b) => b.classList.remove("active"));
      if (isActive) {
        filterExamples(null);
      } else {
        badge.classList.add("active");
        filterExamples(region);
      }
    });
  });
