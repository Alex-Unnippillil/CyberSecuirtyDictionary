...
function updateStreak() {
  ...
  let data;
  try {
    data = JSON.parse(localStorage.getItem("streak")) || {
      count: 0,
      last: null,
    };
  } catch (e) {
    data = { count: 0, last: null };
  }

  let message = "";
  if (data.last === today) {
    message = `🔥 You're on a ${data.count}-day streak!`;
  } else {
    ...
    if (data.last === yesterday) {
      ...
      if (data.count > 0) {
        message = `You broke your ${data.count}-day streak. Start again today!`;
      } else {
  ...
  if (streakReminder) {
    streakReminder.textContent = message;
    streakReminder.style.display = "block";
    setTimeout(() => {
      streakReminder.style.display = "none";
    }, 5000);
  }
}
