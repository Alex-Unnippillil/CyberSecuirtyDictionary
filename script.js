...
const applyDensity = (density) => {
  ...
  if (densityToggle) {
    const label = density === "compact" ? "comfortable" : "compact";
    densityToggle.textContent = `${label.charAt(0).toUpperCase() + label.slice(1)} Mode`;
    densityToggle.setAttribute("aria-label", `Switch to ${label} mode`);
  }
};
...
if (densityToggle) {
  densityToggle.addEventListener("click", () => {
    const newDensity =
      document.body.dataset.density === "compact" ? "comfortable" : "compact";
