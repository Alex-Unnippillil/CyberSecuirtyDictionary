...
function displayDefinition(term) {
  definitionContainer.style.display = "block";
  definitionContainer.innerHTML = `<div class="definition-header"><h3>${term.term}</h3><button type="button" class="copy-btn" aria-label="Copy definition">Copy</button></div><p class="definition-text">${term.definition}</p>`;
  ...
  const copyBtn = definitionContainer.querySelector(".copy-btn");
  const defText = definitionContainer.querySelector(".definition-text");
  copyBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    navigator.clipboard
      .writeText(`${term.term}: ${term.definition}`)
      .then(() => {
        if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
          copyBtn.textContent = "Copied!";
          defText.classList.add("copied-reduced");
          setTimeout(() => {
            copyBtn.textContent = "Copy";
            defText.classList.remove("copied-reduced");
          }, 1000);
        } else {
          copyBtn.classList.add("copied");
          defText.classList.add("copied");
          setTimeout(() => {
            copyBtn.classList.remove("copied");
            defText.classList.remove("copied");
          }, 600);
        }
      })
      .catch(() => {});
