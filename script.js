...
const motionRadios = document.querySelectorAll('input[name="motion-level"]');
const motionKey = "motionLevel";
...
function applyMotion(level) {
  const durations = {
    full: "var(--motion-full)",
    reduced: "var(--motion-reduced)",
    none: "var(--motion-none)",
  };
  document.documentElement.style.setProperty(
    "--motion-duration",
    durations[level] || durations.full,
  );
}

const savedMotion = localStorage.getItem(motionKey) || "full";
applyMotion(savedMotion);
motionRadios.forEach((radio) => {
  radio.checked = radio.value === savedMotion;
  radio.addEventListener("change", () => {
    applyMotion(radio.value);
    try {
      localStorage.setItem(motionKey, radio.value);
    } catch (e) {
