...
function displayDefinition(term) {
  ...
  setupCrossRefs(definitionContainer);
...
function setupCrossRefs(container) {
  const links = container.querySelectorAll('a[href^="#"]');
  links.forEach((link) => {
    link.addEventListener("mouseenter", handleCrossRefEnter);
    link.addEventListener("mouseleave", hideHoverCard);
    link.addEventListener("click", (e) => {
      e.preventDefault();
      const termName = decodeURIComponent(
...
function handleCrossRefEnter(e) {
  const link = e.currentTarget;
  const termName = decodeURIComponent(link.getAttribute("href").substring(1));
  if (!prefetchedTerms.has(termName)) {
    const term = termsData.terms.find(
      (t) => t.term.toLowerCase() === termName.toLowerCase(),
    );
    if (term) {
      prefetchedTerms.set(termName, term.definition);
    }
  }
  const definition = prefetchedTerms.get(termName);
  if (definition) {
    hoverCard.innerHTML = `<strong>${termName}</strong><p>${definition}</p>`;
